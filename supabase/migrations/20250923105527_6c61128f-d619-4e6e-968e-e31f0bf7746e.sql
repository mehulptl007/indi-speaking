...
-- Insert sample sections for each god (fix ambiguous column reference)
INSERT INTO public.god_sections (god_id, section_name, order_index)
SELECT g.id, sections.section_name, sections.section_order FROM public.gods g
CROSS JOIN (
  VALUES 
    ('शक्ति', 1),
    ('परिवार', 2),
    ('भक्त', 3),
    ('श्लोक', 4)
) AS sections(section_name, section_order);
...
SELECT s.id, sections.section_name, sections.section_order FROM public.scriptures s
CROSS JOIN (
  VALUES 
    ('महत्व', 1),
    ('ऐतिहासिक काल', 2)
