...
import { useScriptures, useScriptureSections, useScriptureContent } from "@/hooks/useScriptures";
import { Skeleton } from "@/components/ui/skeleton";

const ScriptureContent = () => {
  const navigate = useNavigate();
  const { scriptureId, sectionId } = useParams<{ scriptureId: string; sectionId: string }>();
  const { scriptures, loading: scripturesLoading } = useScriptures();
  const { sections, loading: sectionsLoading } = useScriptureSections(scriptureId || '');
  const { content, loading: contentLoading, error } = useScriptureContent(scriptureId || '', sectionId || '');

  const scripture = scriptures.find(s => s.id === scriptureId);
  const section = sections.find(s => s.id === sectionId);
  const loading = scripturesLoading || sectionsLoading || contentLoading;
      ...
      <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-accent/5">
        ...
        <div className="sticky top-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border">
          <div className="max-w-4xl mx-auto p-4 flex items-center justify-between">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => navigate(`/scriptures/${scriptureId}`)}
        ...
        <div className="max-w-4xl mx-auto p-4 pt-16 text-center">
          <h1 className="text-3xl font-bold sacred-text mb-4">
            {scripture.name} - {section.section_name}
          </h1>
